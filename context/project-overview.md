...
Ghost AI is a real-time collaborative system design workspace. Users describe a system in plain English, and an AI agent maps that system onto a shared canvas live. Collaborators refine the architecture together, and the app generates a complete technical specification document from the resulting graph.
...
4. Let Al generate an initial architecture from a natural language prompt.
...
5. User prompts the Al to generate or extend the system design.

6. Al generates nodes and edges in the shared canvas.
...
- Covers common patterns: monolith, microservices, event-driven, serverless,
  and more.
...
- Authendication and route protection
...
- Al-powered architecture generation from prompts

- Al-powered Markdown spec generation from the canvas graph
...
- Al can generate an architecture into the shared room from a prompt.
